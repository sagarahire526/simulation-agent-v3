...
# SET
#     n.definition = 'Material Hold Time by GC — measures how long each general contractor is CURRENTLY HOLDING material that has NOT YET reached the construction/swap milestone. This is a still-holding (open) snapshot, NOT a historical / closed-cycle metric.\n\nHARD CONSTRAINTS (the generated SQL MUST satisfy ALL four):\n  1. The chosen anchor column {cx_anchor_col} MUST be IS NULL (the milestone has not happened — material is still being held). Never write IS NOT NULL on the anchor.\n  2. hold_days = CURRENT_DATE - pj_a_3925_msl_pickup_date_finish::date. Never subtract the anchor from pickup — the anchor is NULL by definition, so any subtraction with it would yield NULL.\n  3. Counts MUST use COUNT(DISTINCT pj_project_id). Never use s_site_id for counting.\n  4. NEVER filter on the delay-code column. {delay_code_col} is a SELECT / GROUP BY dimension ONLY — never put it in the WHERE clause. Forbidden predicates include `{delay_code_col} IS NOT NULL`, `{delay_code_col} IS NULL`, `{delay_code_col} <> \'No Delay\'`, and any equality on it. Rows with NULL delay_reason simply form their own bucket in the output.\n\nBy default the anchor is pj_a_4225_construction_start_finish (construction/swap start). When the user asks about hold until completion, the phase filter switches the anchor to pj_a_5175_construction_complete_finish (construction/swap complete). Reports project_count, avg_hold_days, max_hold_days, and 8-day SLA breach counts per region / area / market / GC / delay reason.',
#     n.kpi_business_logic = 'STEP 0 — HARD CONSTRAINTS (these are non-negotiable; the generated SQL MUST satisfy them): (a) {cx_anchor_col} IS NULL — projects must be STILL HOLDING (milestone has not happened). Never IS NOT NULL on the anchor. (b) hold_days = CURRENT_DATE - pj_a_3925_msl_pickup_date_finish::date — never subtract the anchor from pickup; the anchor is NULL by definition so that arithmetic would always yield NULL. (c) Counts use COUNT(DISTINCT pj_project_id) — never s_site_id. (d) NEVER place {delay_code_col} in the WHERE clause. It is a SELECT / GROUP BY column only. Specifically, the WHERE clause MUST NOT contain `{delay_code_col} IS NOT NULL`, `{delay_code_col} IS NULL`, `{delay_code_col} <> \'No Delay\'`, or any equality on it. Rows with NULL delay_reason just form their own bucket in the output. STEP 1 — Source: pwc_macro_staging_schema.stg_ndpd_mbt_tmobile_macro_combined; no JOIN. STEP 2 — Phase filter resolves {cx_anchor_col} and {delay_code_col}. When phase=\'start\' (default): {cx_anchor_col} resolves to pj_a_4225_construction_start_finish, and {delay_code_col} resolves to pj_construction_start_delay_code. When phase=\'complete\': {cx_anchor_col} resolves to pj_a_5175_construction_complete_finish, and {delay_code_col} resolves to pj_construction_complete_delay_code. STEP 3 — Population WHERE clause has exactly TWO predicates against milestone columns: the pickup column pj_a_3925_msl_pickup_date_finish must be IS NOT NULL, AND the anchor column {cx_anchor_col} must be IS NULL. No other milestone-column predicates are added. STEP 4 — Apply optional equality filters: rgn_region, m_area, m_market, construction_gc, pj_project_id, s_site_id, customer_site_code, smp_id, por_category. STEP 5 — smp_name conditional: if smp_name = \'NTM\', enforce smp_status = \'Active\' and exclude ntm_project_type IN (\'E2E06 - NSD Cx Mgmt\', \'E2E07 - OL/MOD Cx Mgmt\'). STEP 6 — Optional date range start_date / end_date applied to pj_a_3925_msl_pickup_date_finish::date. STEP 7 — Aggregate per (rgn_region, m_area, m_market, construction_gc, delay_reason): project_count = COUNT(DISTINCT pj_project_id); avg_hold_days = ROUND(AVG(hold_days)::numeric, 0); max_hold_days = MAX(hold_days); sla_breach_count = COUNT(DISTINCT pj_project_id WHERE hold_days > 8); sla_breach_pct = ROUND(100.0 * sla_breach_count / NULLIF(project_count, 0), 2). STEP 8 — ORDER BY avg_hold_days DESC NULLS LAST, construction_gc, delay_reason.',
