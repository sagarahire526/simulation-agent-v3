...
if __name__ == "__main__":
    ...
    # def _round_displayable(x):
    #     """Round to nearest integer with ties going DOWN, min 1.

    #     Rule: fractional part STRICTLY > 0.5 rounds up; everything else
    #     rounds down. Then floor at 1 so productivity / crew counts never
    #     display as zero.
    ...
    #       1.6  -> 2   (0.6 > 0.5)
    #       1.5  -> 1   (0.5 NOT > 0.5)
    #       0.23 -> 1   (would be 0, floored to 1)
    #     """
    #     try:
    #         x = float(x)
    #     except Exception:
    ...
    #     n = int(x)
    #     frac = x - n
    #     # Use a tiny epsilon to absorb float-precision noise so 1.5000000001
    #     # still counts as > 0.5, but a true 1.5 doesn't.
    #     if frac > 0.5 + 1e-9:
    #         n += 1
