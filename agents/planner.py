...
def _run_scenario_bypass(scenario_match: dict, query: str, project_type: str = "") -> dict | None:
    ...
    try:
        ...
        # The planner just EXECUTES the scenario and passes the structured result through.
        # Rendering + response-facing trimming happen at the response boundary
        # (agents/response.py via agents/scenario_render.py) — the planner stays dumb, and
        # the full untrimmed result is carried through for the client to attach after the LLM.
        step = {
            "traversal_findings": "",           # rendered lazily by the response agent (lean)
            "traversal_tool_calls": [{
                ...
                "tool_output": result,          # full detail — summarized (<=150 chars) for the LLM, not dumped
                "status": "success",
                "execution_time_ms": round(dt, 1),
            }],
            "traversal_steps_taken": 1,
            # Carried through so the response agent can render a LEAN view and the client can
            # attach the FULL detail after the response runs.
