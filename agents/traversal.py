...
def _build_project_type_filter(project_type_raw: str) -> str:
    ...
    return (
        ...
        f'- **Branched reference functions — pick exactly ONE branch**: '
        f'If the reference `kpi_python_function` has `if smp_upper == \'NTM\' / '
        f'\'AHLOB MODERNIZATION\' / else` branches with different SQL bodies '
        f'(e.g. `get_crew_capacity` — NTM uses a 4-batch 39-day rolling window, '
        f'AHLOB uses a 7-day snapshot, default uses no window), copy ONLY the '
        f'branch matching **{display}**. Same column names across branches do '
        f'NOT mean same logic — the `crew_lead_count` from NTM\'s SQL and from '
        f'AHLOB\'s SQL are different numbers. Before writing your SQL, state in '
        f'one line: *"Using the {display} branch of `kpi_python_function`."* '
        f'Then copy that branch\'s tables, joins, date filters, GROUP BY, and '
        f'aggregations as a single unit — do not pull pieces from another branch.\n'
