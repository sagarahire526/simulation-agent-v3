...
def _build_project_type_filter(project_type_raw: str) -> str:
    ...
    return (
        ...
        f'- **⚠️ BRANCHED REFERENCE FUNCTIONS — keep exactly ONE branch**: Some '
        f'reference functions (notably `get_crew_capacity`, and any other KPI whose '
        f'`kpi_python_function` contains `if smp_upper == \'NTM\': ... elif smp_upper '
        f'== \'AHLOB MODERNIZATION\': ... else: ...`) carry DIFFERENT SQL bodies per '
        f'branch — each encoding a different business calculation. Example: the '
        f'`get_crew_capacity` KPI uses a **4-batch 39-day rolling average** for NTM '
        f'(crews rotate weekly), a **last-7-days snapshot** for AHLOB Modernization '
        f'(shorter program cycles), and a **no-date-window default** for everything '
        f'else. These branches are NOT interchangeable retrieval styles — each is '
        f'the correct calculation for ONE project type only.\n'
        f'  Procedure (do this BEFORE writing any SQL):\n'
        f'  1. Identify which branch in the reference function matches the user\'s '
        f'project type **{display}** (compare against each `if smp_upper == \'...\':` '
        f'literal — the user\'s value wins).\n'
        f'  2. STATE your selection out loud, e.g. *"Branch selected: {display} '
        f'block in `kpi_python_function` — matches the user-selected project type. '
        f'Discarding the other branches and the `else` fallback."*\n'
        f'  3. Copy ONLY that branch\'s SQL pattern character-for-character into your '
        f'`run_sql_python` block. Do NOT carry across the other branches\' window '
        f'sizes, batch logic, GROUP BY columns, or table joins. Do NOT use the '
        f'`else` default when a project-type-specific branch matches.\n'
        f'  4. If `{display}` matches NO branch literal in the reference, only then '
        f'is the `else` default correct.\n'
        f'  Why this matters: mixing branches (e.g. running NTM\'s 4-batch window '
        f'with the AHLOB filter, or applying the default no-window SQL when an NTM '
        f'branch exists) produces numerically valid output that answers a DIFFERENT '
        f'question. The PM has no way to detect this kind of error from the response '
        f'alone — it is the most damaging class of hallucination in this system.\n'
